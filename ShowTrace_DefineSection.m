...
%% Section 3: Show activation ROI
file_need{3} = [2];% the idx of file needed
frame_raw_begin{3} = 1;
frame_raw_end{3} = 1;
frame_showfig_amount{3} = 2*fps; %对应要产生多少张图，比上面的张数多的话，会自动匹配
section_char{3} = [3 1 frame_showfig_amount{3}];%%第一段展示那几个字符,字符在后面几行123，每一行是一种，每一行的后两个数字是这个文字出现在fig多少张
crop_info_temp{3} = [526 526 1052 1052];%行表示保持这个crop状态，两行为首位中间状态自动插值。side模式：直接从image-J读的四个参数(起始列，起始行，总列数，总行数)
